  ...
  <script type="text/babel">
    ...
    const parseUserContent = (content) => {
      ...
      if (content.startsWith('[GOMB]')) {
        ...
        if (colonIdx > -1) cleaned = cleaned.slice(colonIdx + 1).trim();
        return '🔘 ' + cleaned;
      }
      return content;
    };
    ...
    function App() {
      ...
      const fetchMessages = useCallback(async () => {
        ...
        try {
          const response = await fetch(
            CONFIG.SUPABASE_URL + '/rest/v1/n8n_chat_histories?select=id,session_id,message,created_at&order=id.asc',
            {
              headers: {
          ...
          const grouped = data.reduce((acc, row) => {
            ...
            if (!acc[phone]) acc[phone] = { messages: [], lastMessageId: 0 };
          ...
          const sortedPhones = Object.keys(grouped).sort((a, b) => grouped[b].lastMessageId - grouped[a].lastMessageId);
          const sorted = {};
          sortedPhones.forEach(p => sorted[p] = grouped[p]);
          
          const currentSelectedCount = selectedPhone && sorted[selectedPhone] ? sorted[selectedPhone].messages.length : 0;
      ...
      const phones = Object.keys(conversations).filter(p => p.toLowerCase().includes(searchTerm.toLowerCase()));

      if (loading && isInitialLoad) {
        return (<div className="min-h-screen bg-gray-900 flex items-center justify-center"><div className="text-center"><div className="text-5xl mb-4 animate-bounce">💬</div><div className="text-white text-lg">Loading...</div></div></div>);
      }

      if (error && Object.keys(conversations).length === 0) {
        return (<div className="min-h-screen bg-gray-900 flex items-center justify-center p-4"><div className="bg-red-500/20 border border-red-500 rounded-xl p-6 max-w-md text-center"><div className="text-4xl mb-3">⚠️</div><div className="text-red-300 text-lg mb-2">Error</div><div className="text-red-400 text-sm">{error}</div><button onClick={fetchMessages} className="mt-4 bg-red-600 hover:bg-red-500 text-white px-4 py-2 rounded-lg">Retry</button></div></div>);
      }
        ...
        <div className="h-screen bg-gray-900 flex overflow-hidden">
          <div className={(mobileShowChat ? 'hidden md:flex' : 'flex') + ' w-full md:w-80 bg-gray-800 border-r border-gray-700 flex-col'}>
            ...
            <div className="flex-1 overflow-y-auto scrollbar-thin">
              {phones.length === 0 ? (<div className="p-4 text-center text-gray-500">No conversations</div>) : phones.map(phone => (
                <div key={phone} onClick={() => handleSelectPhone(phone)} className={'p-4 border-b border-gray-700/50 cursor-pointer transition-colors ' + (selectedPhone === phone ? 'bg-gray-700/70' : 'hover:bg-gray-700/30')}>
                  <div className="flex items-center gap-3">
                    <div className="w-12 h-12 rounded-full bg-gradient-to-br from-blue-500 to-blue-600 flex items-center justify-center text-white font-semibold shadow-lg flex-shrink-0">{phone.slice(-2)}</div>
            ...
            <div className="p-3 border-t border-gray-700 text-xs text-gray-500 text-center">{lastRefresh ? formatTime(lastRefresh) : ''}</div>
          </div>
            ...
            {selectedPhone ? (
              <>
                <div className="px-4 py-4 border-b border-gray-700 bg-gray-800/50 flex items-center gap-3">
                  <button onClick={() => setMobileShowChat(false)} className="md:hidden p-2 -ml-2 text-gray-400"><svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" /></svg></button>
                  ...
                  <div><div className="font-semibold text-white">{formatPhone(selectedPhone)}</div><div className="text-xs text-gray-400">{conv ? conv.messages.length : 0} messages</div></div>
                </div>
                        ...
                        <div className="max-w-[85%] md:max-w-[70%]">
                          <div className={'rounded-2xl px-4 py-2.5 shadow-md ' + (isAi ? 'bg-gray-700 text-white rounded-bl-sm' : 'bg-blue-600 text-white rounded-br-sm')}>
                            <div className={'text-xs mb-1 flex justify-between ' + (isAi ? 'text-gray-400' : 'text-blue-200')}>
                              ...
                              <span className="ml-3">{formatTime(msg.created_at)}</span>
                            </div>
                            <p className="text-sm whitespace-pre-wrap break-words">{msg.content}</p>
                          </div>
                          {isAi && msg.buttons && msg.buttons.length > 0 && (
                            <div className="mt-2 flex flex-wrap gap-2">{msg.buttons.map((btn, bi) => (<span key={bi} className="inline-block bg-gray-600 text-white text-xs px-3 py-1.5 rounded-full border border-gray-500">{btn}</span>))}</div>
                          )}
                        </div>
                      </div>
                    );
                ...
                <div className="px-4 py-3 border-t border-gray-700 bg-gray-800/50"><div className="bg-gray-700/50 rounded-full px-4 py-3 text-gray-500 text-sm text-center">👀 View-only</div></div>
              </>
            ) : (
              <div className="flex-1 flex items-center justify-center text-gray-500"><div className="text-center"><div className="text-6xl mb-4">💬</div><p>Select a conversation</p></div></div>
            )}
          </div>
        </div>
      );
...
server.listen(PORT, () => console.log('Callera Inbox on port ' + PORT));
